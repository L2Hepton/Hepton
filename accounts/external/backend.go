...
func (api *ExternalSigner) SignData(account accounts.Account, mimeType string, data []byte) ([]byte, error) {
	...
	// If V is on 27/28-form, convert to 0/1 for Clique/Hapten
	if (mimeType == accounts.MimetypeClique || mimeType == accounts.MimetypeHapten) && (res[64] == 27 || res[64] == 28) {
		res[64] -= 27 // Transform V from 27/28 to 0/1 for Clique/Hapten use
	}
	return res, nil
}
