...
func loadSnapshot(config *params.ChainConfig, sigcache *lru.ARCCache, db ethdb.Database, hash common.Hash) (*Snapshot, error) {
	blob, err := db.Get(append([]byte("hapten-"), hash[:]...))
	if err != nil {
		return nil, err
	}
	snap := new(Snapshot)
...
func (s *Snapshot) store(db ethdb.Database) error {
	...
	return db.Put(append([]byte("hapten-"), s.Hash[:]...), blob)
}
...
func (s *Snapshot) SignedRecently(block uint64, validator common.Address) bool {
	continuousInturn := s.config.HaptenContinuousInturn(big.NewInt(int64(block)))
...
func (s *Snapshot) apply(headers []*types.Header, chain consensus.ChainHeaderReader, parents []*types.Header) (*Snapshot, error) {
	...
	for i, header := range headers {
		...
		continuousInturn := s.config.HaptenContinuousInturn(header.Number)
		...
		if number > 0 && number%s.config.Hapten.Epoch == 0 {
			...
			if uint64(i) >= s.config.Hapten.Epoch {
				checkpointHeader = headers[i-int(s.config.Hapten.Epoch)]
			} else {
				// i < epoch ==> epoch -i >= 1
				idxInParents := len(parents) - (int(s.config.Hapten.Epoch) - i)
				if idxInParents >= 0 {
					checkpointHeader = parents[idxInParents]
				} else {
					checkpointHeader = chain.GetHeaderByNumber(number - s.config.Hapten.Epoch)
...
func (s *Snapshot) inturn(number uint64, validator common.Address) bool {
	...
	continousInturn := s.config.HaptenContinuousInturn(big.NewInt(int64(number)))
