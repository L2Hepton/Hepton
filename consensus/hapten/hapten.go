...
// Package hapten implements the proof-of-stake-authority consensus engine.
package hapten
...
import (
	...
	"github.com/ethereum/go-ethereum/consensus/hapten/systemcontract"
...
// Hapten proof-of-stake-authority protocol constants.
var (
...
// Hapten is a delegate proof-of-stake consensus engine
type Hapten struct {
	chainConfig *params.ChainConfig // ChainConfig to execute evm
	config      *params.HaptenConfig // Consensus engine configuration parameters
...
// New creates a Hapten proof-of-stake-authority consensus engine with the initial
// validators set to the ones provided by the user.
func New(chainConfig *params.ChainConfig, db ethdb.Database) *Hapten {
	// Set any missing consensus parameters to their defaults
	conf := *chainConfig.Hapten
	if conf.Epoch <= 1 {
...
func (c *Hapten) GetDb() ethdb.Database {
	return c.db
}

func (c *Hapten) SetChain(chain consensus.ChainHeaderReader) {
	c.chain = chain
}

// SetStateFn sets the function to get state.
func (c *Hapten) SetStateFn(fn StateFn) {
	c.stateFn = fn
}

// InitRewardsUpdatePeroid init rewardsUpdatePeroid by reading from system contract,
func (c *Hapten) InitRewardsUpdatePeroid(chain consensus.ChainHeaderReader, state *state.StateDB) error {
...
func (c *Hapten) Author(header *types.Header) (common.Address, error) {
	return header.Coinbase, nil
	// return ecrecover(header, c.signatures)
}

// VerifyHeader checks whether a header conforms to the consensus rules.
func (c *Hapten) VerifyHeader(chain consensus.ChainHeaderReader, header *types.Header, seal bool) error {
...
func (c *Hapten) VerifyHeaders(chain consensus.ChainHeaderReader, headers []*types.Header, seals []bool) (chan<- struct{}, <-chan error) {
...
func (c *Hapten) verifyHeader(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Hapten) verifyCascadingFields(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Hapten) snapshot(chain consensus.ChainHeaderReader, number uint64, hash common.Hash, parents []*types.Header) (*Snapshot, error) {
...
func (c *Hapten) VerifyUncles(chain consensus.ChainReader, block *types.Block) error {
...
func (c *Hapten) VerifySeal(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// from.
func (c *Hapten) verifySeal(chain consensus.ChainHeaderReader, header *types.Header, parents []*types.Header) error {
...
func (c *Hapten) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
...
// rewards given.
func (c *Hapten) Finalize(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB,
...
func (c *Hapten) FinalizeAndAssemble(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB,
...
// * update rewards info
// * punish double sign
func (c *Hapten) prepareFinalize(chain consensus.ChainHeaderReader, header *types.Header,
...
func (c *Hapten) updateValidators(vmCtx *systemcontract.CallContext, chain consensus.ChainHeaderReader, mined bool) error {
...
func (c *Hapten) tryDistributeBlockFee(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
...
func (c *Hapten) tryLazyPunish(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	number := header.Number.Uint64()
	snap, err := c.snapshot(chain, number-1, header.ParentHash, nil)
	if err != nil {
		return err
	...
	continuousBlocks := c.chainConfig.HaptenContinuousInturn(header.Number)
...
func (c *Hapten) getTopValidators(chain consensus.ChainHeaderReader, header *types.Header) ([]common.Address, error) {
...
func (c *Hapten) Authorize(validator common.Address, signFn ValidatorFn, signTxFn SignTxFn) {
	c.lock.Lock()
	defer c.lock.Unlock()
...
func (c *Hapten) punishDoubleSign(chain consensus.ChainHeaderReader, header *types.Header,
	state *state.StateDB, txs *[]*types.Transaction, receipts *[]*types.Receipt, punishTxs []*types.Transaction, mined bool) error {
	if !mined {
...
func (c *Hapten) AttestationStatus() uint8 {
	return c.attestationStatus
}

func (c *Hapten) StartAttestation() {
	c.attestationStatus = types.AttestationStart
}

func (c *Hapten) StopAttestation() {
	c.attestationStatus = types.AttestationStop
}

func (c *Hapten) ClearAllViolateCasperFFGPunish() {
	rawdb.ClearAllViolateCasperFFGPunish(c.db)
}

// Seal implements consensus.Engine, attempting to create a sealed block using
// the local signing credentials.
func (c *Hapten) Seal(chain consensus.ChainHeaderReader, block *types.Block, results chan<- *types.Block, stop <-chan struct{}) error {
	header := block.Header()
	...
	// Sign all the things!
	sighash, err := signFn(accounts.Account{Address: val}, accounts.MimetypeHapten, HaptenRLP(header))
	if err != nil {
		return err
	}
...
func (c *Hapten) CalcDifficulty(chain consensus.ChainHeaderReader, time uint64, parent *types.Header) *big.Int {
...
func (c *Hapten) SealHash(header *types.Header) common.Hash {
	return SealHash(header)
}

// Close implements consensus.Engine. It's a noop for hapten as there are no background threads.
func (c *Hapten) Close() error {
	return nil
}

// APIs implements consensus.Engine, returning the user facing RPC API to allow
// controlling the validator voting.
func (c *Hapten) APIs(chain consensus.ChainHeaderReader) []rpc.API {
	return []rpc.API{{
		Namespace: "hapten",
		Version:   "1.0",
		Service:   &API{chain: chain, hapten: c},
		Public:    false,
	}}
}
...
// HaptenRLP returns the rlp bytes which needs to be signed for the proof-of-stake-authority
...
func HaptenRLP(header *types.Header) []byte {
	b := new(bytes.Buffer)
...
func (c *Hapten) PreHandle(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB) error {
	return nil
}

// CalculateGasPool determines gas limit of each block
func (c *Hapten) CalculateGasPool(header *types.Header) uint64 {
