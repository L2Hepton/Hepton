...
func (c *Hapten) VerifyAttestation(chain consensus.ChainHeaderReader, a *types.Attestation) (common.Address, int, error) {
...
func (c *Hapten) CurrentValidator() common.Address {
	return c.validator
}

func (c *Hapten) MaxValidators() uint8 {
	return systemcontract.TopValidatorNum
}

func (c *Hapten) Attest(chain consensus.ChainHeaderReader, headerNum *big.Int, source, target *types.RangeEdge) (*types.Attestation, error) {
	if !c.IsReadyAttest() {
...
func (c *Hapten) makeNewAttestation(sourceRangeEdge *types.RangeEdge, targetRangeEdge *types.RangeEdge) (*types.Attestation, error) {
...
func (c *Hapten) IsAuthorizedAtHeightAndHash(chain consensus.ChainHeaderReader, val common.Address, hash common.Hash, number uint64) bool {
	h := chain.GetHeader(hash, number)
	return c.IsAuthorizedByHeader(chain, val, h)
}

func (c *Hapten) IsAuthorizedAtHeight(chain consensus.ChainHeaderReader, val common.Address, height uint64) bool {
	h := chain.GetHeaderByNumber(height)
	return c.IsAuthorizedByHeader(chain, val, h)
}

func (c *Hapten) IsAuthorizedByHeader(chain consensus.ChainHeaderReader, val common.Address, h *types.Header) bool {
	if h == nil {
...
func (c *Hapten) CurrentNeedHandleHeight(headerNum uint64) (uint64, error) {
...
func (c *Hapten) AttestationDelay() uint64 {
	return c.config.AttestationDelay
}

func (c *Hapten) IsReadyAttest() bool {
	return c.isReady && c.attestationStatus == types.AttestationStart
}

func (c *Hapten) AttestationThreshold(chain consensus.ChainHeaderReader, hash common.Hash, number uint64) (int, error) {
...
func (c *Hapten) Validators(chain consensus.ChainHeaderReader, hash common.Hash, number uint64) ([]common.Address, error) {
	header := chain.GetHeader(hash, number)
	if header == nil {
...
func (c *Hapten) VerifyCasperFFGRule(beforeSourceNum uint64, beforeTargetNum uint64, afterSourceNum uint64, afterTargetNum uint64) int {
...
func (c *Hapten) executeDoubleSignPunish(chain consensus.ChainHeaderReader, header *types.Header,
...
func (c *Hapten) replayDoubleSignPunish(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, totalTxIndex int, tx *types.Transaction) (*types.Receipt, error) {
...
func (c *Hapten) IsDoubleSignPunished(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, punishHash common.Hash) (bool, error) {
...
func (c *Hapten) executeDoubleSignPunishMsg(chain consensus.ChainHeaderReader, header *types.Header, state *state.StateDB, p *types.ViolateCasperFFGPunish, totalTxIndex int, txHash, bHash common.Hash) (*types.Receipt, error) {
...
func (c *Hapten) IsDoubleSignPunishTransaction(sender common.Address, tx *types.Transaction, header *types.Header) (bool, error) {
...
func (c *Hapten) ApplyDoubleSignPunishTx(evm *vm.EVM, sender common.Address, tx *types.Transaction) (ret []byte, vmerr error, err error) {
