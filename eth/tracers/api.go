...
type API struct {
	...
	isHaptenEngine bool
	haptenEngine   consensus.HaptenEngine
}

// NewAPI creates a new API definition for the tracing methods of the Ethereum service.
func NewAPI(backend Backend) *API {
	haptenEngine, isHaptenEngine := backend.Engine().(consensus.HaptenEngine)
	return &API{backend: backend, isHaptenEngine: isHaptenEngine, haptenEngine: haptenEngine}
}
...
type txTraceTask struct {
	...
	isDoubleSignPunishTx bool           // Is hapten punish double sign transaction
}
	...
	for th := 0; th < threads; th++ {
		...
		go func() {
			...
			for task := range tasks {
				...
				if api.isHaptenEngine {
					_ = api.haptenEngine.PreHandle(api.backend.ChainHeaderReader(), header, task.statedb)
				}
				...
				for i, tx := range task.block.Transactions() {
					...
					if api.isHaptenEngine {
						isDoubleSignPunishTx, _ = api.haptenEngine.IsDoubleSignPunishTransaction(msg.From(), tx, header)
					}
					if isDoubleSignPunishTx {
						res, err = api.traceHaptenApplyDoubleSignPunishTx(ctx, msg.From(), tx, txctx, blockCtx, task.statedb, config)
					} else {
...
func (api *API) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	if api.isHaptenEngine {
		_ = api.haptenEngine.PreHandle(api.backend.ChainHeaderReader(), header, statedb)
	}
	...
	for i, tx := range txs {
		...
		if api.isHaptenEngine {
			...
			isDoubleSignPunishTx, _ = api.haptenEngine.IsDoubleSignPunishTransaction(sender, tx, header)
		}
		...
		statedb.Prepare(tx.Hash(), i)
		if isDoubleSignPunishTx {
			if _, _, err := api.haptenEngine.ApplyDoubleSignPunishTx(vmenv, msg.From(), tx); err != nil {
				failed = err
				break
			}
			continue
...
func (api *API) standardTraceBlockToFile(ctx context.Context, block *types.Block, config *StdTraceConfig) ([]string, error) {
	...
	for i, tx := range block.Transactions() {
		...
		if isDoubleSignPunishTx {
			_, _, err = api.haptenEngine.ApplyDoubleSignPunishTx(vmenv, msg.From(), tx)
		} else {
...
func (api *API) TraceTransaction(ctx context.Context, hash common.Hash, config *TraceConfig) (interface{}, error) {
	...
	if api.isHaptenEngine {
		...
		if ok, _ := api.haptenEngine.IsDoubleSignPunishTransaction(msg.From(), tx, block.Header()); ok {
			return api.traceHaptenApplyDoubleSignPunishTx(ctx, msg.From(), tx, txctx, vmctx, statedb, config)
		}
	}
...
// TODO Reuse code
func (api *API) traceHaptenApplyDoubleSignPunishTx(ctx context.Context, sender common.Address, tx *types.Transaction, txctx *Context, vmctx vm.BlockContext, statedb *state.StateDB, config *TraceConfig) (interface{}, error) {
	// Assemble the structured logger or the JavaScript tracer
	var (
		tracer vm.EVMLogger
		err    error
	...
	// Call Prepare to clear out the statedb access list
	statedb.Prepare(txctx.TxHash, txctx.TxIndex)
	ret, vmerr, err := api.haptenEngine.ApplyDoubleSignPunishTx(vmenvWithoutTxCtx, sender, tx)
	if err != nil {
		return nil, fmt.Errorf("tracing failed: %w", err)
	}
