...
func (eth *Ethereum) stateAtTransaction(block *types.Block, txIndex int, reexec uint64) (core.Message, vm.BlockContext, *state.StateDB, error) {
	...
	if err == nil && eth.isHaptenEngine {
		err = eth.haptenEngine.PreHandle(eth.blockchain, block.Header(), statedb)
	}
	if err != nil {
	...
	for idx, tx := range block.Transactions() {
		...
		if idx == txIndex {
			// Notice: for a hapten system transaction, the `msg` and `context` should not be used
		...
		if eth.isHaptenEngine {
			sender, _ := types.Sender(signer, tx)
			ok, _ := eth.haptenEngine.IsDoubleSignPunishTransaction(sender, tx, header)
			if ok {
				if _, _, err := eth.haptenEngine.ApplyDoubleSignPunishTx(vmenv, sender, tx); err != nil {
