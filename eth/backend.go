...
import (
	...
	"github.com/ethereum/go-ethereum/consensus/hapten"
...
func New(stack *node.Node, config *ethconfig.Config) (*Ethereum, error) {
	...
	eth.haptenEngine, eth.isHaptenEngine = eth.engine.(consensus.HaptenEngine)
	...
	// do some extra work if consensus engine is hapten.
	if haptenEngine, ok := eth.engine.(*hapten.Hapten); ok {
		// set state fn
		haptenEngine.SetStateFn(eth.blockchain.StateAt)
		// Init RewardsUpdatePeroid
		currState, err := eth.blockchain.State()
		if err != nil {
			return nil, err
		}
		if err = haptenEngine.InitRewardsUpdatePeroid(eth.blockchain, currState); err != nil {
			log.Error("Init RewardsUpdatePeroid failed in Hapten", "err", err)
			return nil, err
		}
	}
...
func (s *Ethereum) shouldPreserve(block *types.Block) bool {
	...
	if _, ok := s.engine.(*hapten.Hapten); ok {
		return false
	}
...
func (s *Ethereum) StartMining(threads int) error {
	...
	if !s.IsMining() {
		...
		if hapten, ok := s.engine.(*hapten.Hapten); ok {
			wallet, err := s.accountManager.Find(accounts.Account{Address: eb})
			if wallet == nil || err != nil {
				log.Error("Etherbase account unavailable locally", "err", err)
				return fmt.Errorf("signer missing: %v", err)
			}
			hapten.Authorize(eb, wallet.SignData, wallet.SignTx)
		}
...
func (s *Ethereum) StartAttestation() {
	if s.IsMining() {
		if c, ok := s.engine.(*hapten.Hapten); ok {
