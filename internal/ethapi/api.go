...
func (s *PublicBlockChainAPI) GetDoubleSignPunishTransactionsByBlockNumber(ctx context.Context, number rpc.BlockNumber) ([]*RPCTransaction, error) {
	haptenEngine, isHaptenEngine := s.b.Engine().(consensus.HaptenEngine)
	if !isHaptenEngine {
		return nil, errors.New("not a HaptenEngine engine")
	}
	...
	return s.getDoubleSignPunishTransactions(block, haptenEngine)
}
...
func (s *PublicBlockChainAPI) getDoubleSignPunishTransactions(block *types.Block, haptenEngine consensus.HaptenEngine) ([]*RPCTransaction, error) {
	header := block.Header()
	bhash := block.Hash()
	...
	for i, tx := range txs {
		...
		if yes, _ := haptenEngine.IsDoubleSignPunishTransaction(sender, tx, header); yes {
