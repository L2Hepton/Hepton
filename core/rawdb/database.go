...
func InspectDatabase(db ethdb.Database, keyPrefix, keyStart []byte) error {
	...
	for it.Next() {
		...
		case bytes.HasPrefix(key, []byte("hapten-")) && len(key) == 6+common.HashLength:
			haptenSnaps.Add(size)
	...
	stats := [][]string{
		...
		{"Key-Value store", "Hapten snapshots", haptenSnaps.Size(), haptenSnaps.Count()},
