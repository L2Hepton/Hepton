...
func NewBlockChain(db ethdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config, shouldPreserve func(block *types.Block) bool, txLookupLimit *uint64) (*BlockChain, error) {
	...
	bc.HaptenEngine, bc.isHaptenEngine = engine.(consensus.HaptenEngine)
	if bc.isHaptenEngine {
		// load stored last attested number
		currentAttested := rawdb.ReadLastAttestNumber(bc.db, bc.HaptenEngine.CurrentValidator())
	...
	if isNeedReorg == NotSure {
		...
		if !reorg && externTd.Cmp(localTd) == 0 {
			...
			if block.NumberU64() < currentBlock.NumberU64() {
				...
				// Mainly for hapten engine, we use a more certainty rule to select a canonical chain, which is:
