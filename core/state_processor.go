...
func (p *StateProcessor) Process(block *types.Block, statedb *state.StateDB, cfg vm.Config) (types.Receipts, []*types.Log, uint64, error) {
	...
	haptenEngine, isHaptenEngine := p.engine.(consensus.HaptenEngine)
	if isHaptenEngine {
		if err := haptenEngine.PreHandle(p.bc, header, statedb); err != nil {
	...
	for i, tx := range block.Transactions() {
		...
		if isHaptenEngine {
			...
			ok, err := haptenEngine.IsDoubleSignPunishTransaction(sender, tx, header)
			if err != nil {
				return nil, nil, 0, err
			}
			if ok {
