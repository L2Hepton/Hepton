...
func (bc *BlockChain) bestAttestationToProcessed(headNum *big.Int) (*types.Attestation, error) {
	currentNeedHandleHeight, err := bc.HaptenEngine.CurrentNeedHandleHeight(headNum.Uint64())
	if err != nil {
		return nil, err
	}
	...
	if currentNeedHandleHeight > unableSureBlockStateInterval {
		...
		if re.Number.Uint64() <= diffNumber {
			...
			return bc.HaptenEngine.Attest(bc, new(big.Int).SetUint64(currentNeedHandleHeight), source, target)
		}
	}
	// Fast update
	...
	return bc.HaptenEngine.Attest(bc, new(big.Int).SetUint64(currentNeedHandleHeight), re, target)
}
...
func (bc *BlockChain) processAttestationOnHead(head *types.Header) {
	if bc.HaptenEngine.AttestationStatus() == types.AttestationPending {
		// Give priority to judge whether it has caught up
		firstCatchup := bc.firstCatchUpNumber.Load().(*big.Int)
		attestationDelay := bc.HaptenEngine.AttestationDelay()
		// Prevent false triggering during node initialization
		continuousBlocks := bc.chainConfig.HaptenContinuousInturn(head.Number)
		...
		if firstCatchup.Uint64() > 0 && head.Number.Uint64() > firstCatchup.Uint64()+attestationDelay &&
			...
			bc.HaptenEngine.StartAttestation()
	...
	if bc.HaptenEngine.IsReadyAttest() {
		...
		threshold, err := bc.HaptenEngine.AttestationThreshold(bc, a.TargetRangeEdge.Hash, a.TargetRangeEdge.Number.Uint64())
		if err != nil {
			log.Error(err.Error())
			return
		}
		err = bc.AddOneValidAttestationToRecentCache(a, threshold, bc.HaptenEngine.CurrentValidator())
		if err != nil {
			log.Error(err.Error())
			return
		}
...
func (bc *BlockChain) StoreLastAttested(num *big.Int) {
	...
	rawdb.WriteLastAttestNumber(bc.db, bc.HaptenEngine.CurrentValidator(), num)
...
func (bc *BlockChain) AddOneAttestationToRecentCache(a *types.Attestation, signer common.Address, isTest bool) error {
	...
	_, threshold, err := bc.HaptenEngine.VerifyAttestation(bc, a)
	if err != nil && !isTest {
		return err
	}
...
func (bc *BlockChain) VerifyCasperFFGRecentCache(a *types.Attestation, signer common.Address) error {
	...
	if found {
		...
		for _, h := range cfhList {
			ruleType := bc.HaptenEngine.VerifyCasperFFGRule(a.SourceRangeEdge.Number.Uint64(), a.TargetRangeEdge.Number.Uint64(),
...
func (bc *BlockChain) ViolationCasperFFGExecutePunish(before *types.Attestation, after *types.Attestation, punishType int, blockNum *big.Int) error {
	return rawdb.WriteViolateCasperFFGPunish(bc.HaptenEngine.GetDb(), before, after, punishType, blockNum)
}
...
func (bc *BlockChain) CalculateCurrentEpochIndex(number uint64) uint64 {
	return number / bc.chainConfig.Hapten.Epoch
}
...
func (bc *BlockChain) UpdateCurrentEpochBPList(hash common.Hash, number uint64) error {
	...
	if last == nil || last.CurrentEpochIndex.Uint64() < newCurrentEpochIndex {
		bps, err := bc.HaptenEngine.Validators(bc, hash, number)
		if err != nil {
			return err
		}
		...
		if last != nil {
			...
			epoch := bc.chainConfig.Hapten.Epoch
			if number > epoch {
				block := bc.GetBlockByNumber(number - epoch)
				lastBps, err := bc.HaptenEngine.Validators(bc, block.Hash(), block.NumberU64())
				if err != nil {
					return err
				}
...
func (bc *BlockChain) MaxValidators() uint8 {
	return bc.HaptenEngine.MaxValidators()
}
