...
func newWorker(config *Config, chainConfig *params.ChainConfig, engine consensus.Engine, eth Backend, mux *event.TypeMux, isLocalBlock func(*types.Block) bool, init bool) *worker {
	haptenEngine, isHaptenEngine := engine.(consensus.HaptenEngine)
	worker := &worker{
		config:             config,
		chainConfig:        chainConfig,
		engine:             engine,
		isHaptenEngine:      isHaptenEngine,
		haptenEngine:        haptenEngine,
		eth:                eth,
		mux:                mux,
...
func (w *worker) commitTransactions(txs *types.TransactionsByPriceAndNonce, coinbase common.Address, interrupt *int32) bool {
	...
	if w.current.gasPool == nil {
		if w.isHaptenEngine {
			w.current.gasPool = new(core.GasPool).AddGas(w.haptenEngine.CalculateGasPool(w.current.header))
		} else {
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	env := w.current
	if w.isHaptenEngine {
		if err := w.haptenEngine.PreHandle(w.chain, header, env.state); err != nil {
